 ...
 */
    class FuzzySet {
    public:
        using ComposedMembershipFunction = std::function<double(std::map<std::string, double>)>;
         ...
         */
        friend std::shared_ptr<FuzzySet>
        operator||(const std::shared_ptr<FuzzySet> &lhs, const std::shared_ptr<FuzzySet> &rhs);
