...
\section{Conversion of a Decision Tree into a Fuzzy Inference System}
...
\noindent The exact shape of the decision surface depends on the used defuzzification method. The most common choice is the \gls{cog} method, which calculates the $x$-position of the center of gravity of the resulting membership function.
However, using the \gls{cog} method can lead to undesired results when using nominal \todo{check} values for the output classes, as there is no concept of ordering among the values. In such cases, the interpolation between the different classes performed by the \gls{cog} method is not meaningful. In such cases, the \gls{mom} method can be used, which calculates the mean value of the maximum membership functions. In most cases this method will return exactly the membership function with the highest value, which is the only meaningful result in this case. The decision surfaces of the fuzzy system created from the fuzzy decision tree in \autoref{fig:fuzzyDecisionTreeExample} using the \gls{cog} and \gls{mom} methods are shown in \autoref{fig:fuzzyDecisionSurfaceExampleCOG} and \autoref{fig:fuzzyDecisionSurfaceExampleMOM}. This also marks the end of the conversion process, since we now have a complete fuzzy system that can be used to predict new data points based on their features.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \begin{scope}[x={(image.south east)},y={(image.north west)}]
                \draw[yellow, thin,rounded corners] (.52,.67) rectangle (.62,.95);
                \draw[yellow, thin,rounded corners] (.56,.3) rectangle (.74,.46);
                \node[yellow,anchor=north] (A) at (.45,.25) {\tiny{Interpolation Error}};

                \draw[yellow, arrow] (A) -- (.52,.67);
                \draw[yellow, arrow] (A) -- (.56,.3);
            \end{scope}
        \end{tikzpicture}

        \caption[Decision surface of the fuzzy rules using COG method]{The decision surface of the \gls{fis} created from the fuzzy decision tree in \autoref{fig:fuzzyDecisionTreeExample} over $\mathcal{D}=[0,4]\times[0,3]$ using the \gls{cog} defuzzification method.}
