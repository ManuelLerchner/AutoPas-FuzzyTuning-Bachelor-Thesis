...
\begin{multicols}{2}
    ...
    \begin{figure}[H]
        ...
        \end{tikzpicture}
        \caption[Decision boundary of the example decision tree]{The decision boundary of the decision tree from Figure~\ref{fig:decisionTreeExample} on $\mathcal{D}=[0,4]\times[0,3]$. }
...
However, using the \texttt{COG} method can lead to undesired results when using nominal \todo{check} values for the output classes, as there is no concept of ordering among the values. In such cases, the interpolation between the different classes performed by the \texttt{COG} method is not meaningful. In such cases, the \texttt{MOM} method can be used, which calculates the mean value of the maximum membership functions. In most cases this method will return exactly the membership function with the highest value, which is the most meaningful result in this case. The decision surfaces of the fuzzy system created from the fuzzy decision tree in Figure~\ref{fig:fuzzyDecisionTreeExample} using the \texttt{COG} and \texttt{MOM} methods are shown in Figure~\ref{fig:fuzzyDecisionSurfaceExampleCOG} and Figure~\ref{fig:fuzzyDecisionSurfaceExampleMOM}.


\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=2,font=\tiny]
        \node [rectangle,rounded corners,draw,inner sep=10pt] (A) at (0,0) {
            FIS
        };

        \draw[->] (-0.6,+0.2) -- (A) node [left, above left, pos=0] {$x$};
        \draw[->] (-0.6,-0.2) -- (A) node [left, below left, pos=0] {$y$};


        \draw[->] (A) -- (0,1) node [left, below left, pos=0] {$class$};

        \node [rectangle,rounded corners,draw,inner sep=2pt] (B) at (-1,-2) {
            \begin{tikzpicture}[scale=2,font=\tiny]
                \begin{axis}%
                    [
                        width=4.25cm,
                    ...
                    \addplot[blue] {gaussian(x,1,1)};
                    \addplot[yellow] {gaussian(x,2,0.2)};
                    \addplot[green] {gaussian(x,3,0.2)};
                \end{axis}
            \end{tikzpicture}
...
\begin{multicols}{2}
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            \node[anchor=south west,inner sep=0] (image) at (0,0) { \includegraphics[width=.9\columnwidth]{figures/ProofOfConcepts/fuzzy_system_cog.png}};
            \begin{scope}[x={(image.south east)},y={(image.north west)}]
                \draw[yellow, thin,rounded corners] (.5,.58) rectangle (.6,.86);
                \draw[yellow, thin,rounded corners] (.56,.26) rectangle (.74,.42);
                \node[yellow,anchor=north] (A) at (.45,.25) {\tiny{Interpolation Error}};

                \draw[yellow, arrow] (A) -- (.5,.58);
                \draw[yellow, arrow] (A) -- (.56,.34);
            \end{scope}
        \end{tikzpicture}


        \caption[Decision surface of the fuzzy rules using \texttt{COG} method]{The decision surface of the fuzzy system created from the fuzzy decision tree in Figure~\ref{fig:fuzzyDecisionTreeExample} over $\mathcal{D}=[0,4]\times[0,3]$. Defuzzification is done using the \texttt{COG} method. One can clearly see the interpolation errors introduced by the \texttt{COG} method.}
    ...
    \begin{figure}[H]
        \begin{tikzpicture}
            \node[anchor=south west,inner sep=0] (image) at (0,0) { \includegraphics[width=.9\columnwidth]{figures/ProofOfConcepts/fuzzy_system_mom.png}};
        \end{tikzpicture}
